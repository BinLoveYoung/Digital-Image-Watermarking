function [ crop_IM ] = cropping(A,S,W,W_IM ,a,alpha,Uw,Vw )
crop_IM = W_IM;
for i=1:50
    for j=1:50
        crop_IM(i,:) = 0;
...
% peaksnr_crop=psnr(uint8(crop_IM),A);
% ssimval_crop = ssim(uint8(crop_IM),A);
% psnr_watermark_blur = psnr(E_CW,W)
% ssim_watermark_blur = ssim(E_CW,W)
peaksnr_crop_wm = psnr(EX_CR,uint8(W))
peaksnr_crop=psnr(crop_IM,uint8(A))
ssimval_crop=ssim(crop_IM,uint8(A))
end